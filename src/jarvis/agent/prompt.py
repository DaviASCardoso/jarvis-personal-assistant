...
- `act`     — uma capacidade listada em `available_capabilities` deveria ser
  executada. Nunca proponha uma skill que não esteja nessa lista.
...
5. Responda em português do Brasil, no mesmo registro do usuário.
"""
...
class Capability:
    ...
    Na Fase 4 a lista chega **vazia**: não existe Skill Registry. O modelo é
    informado disso nas `constraints`, para que não proponha `act` no vazio.
    """
...
class PromptBuilder:
    ...
    def _trim(self, envelope: ReasoningEnvelope) -> _Trimmed:
        ...
        → memórias de menor score → truncagem por item (feita na renderização).
        ...
        while True:
            ...
            if len(_dump(self._render(envelope, trimmed))) <= budget.max_envelope_chars:
                if dropped_turns or dropped_events or dropped_memories:
                    logger.debug(
    ...
    def _render(self, envelope: ReasoningEnvelope, trimmed: _Trimmed) -> dict[str, object]:
        ...
        return {
            ...
            "available_capabilities": [
                {"name": item.name, "summary": item.summary} for item in envelope.capabilities
            ],
            "constraints": _render_constraints(envelope, trimmed),
        }
...
def _render_constraints(envelope: ReasoningEnvelope, trimmed: _Trimmed) -> dict[str, object]:
    return {
        "decision_types": list(_DECISION_TYPES),
        "capabilities_available": bool(envelope.capabilities),
        "execution_note": (
